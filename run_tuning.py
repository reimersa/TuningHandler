...
def run_ber_scan(module, chip, tap_settings=[], mode='time', value=10):

    # first, enable TAP1, TAP2
    reset_singleQuad_xml_files(type='ber', modules=[module], chips = [0])
    prepare_singleQuad_xml_files(type_name='ber', type_setting='scurve', modules=[module])
    xmlfile_for_ber = os.path.join(xmlfolder, 'CMSIT_singleQuad_%s_%s.xml' % (module, 'ber'))
    xmlobject = XMLInfo(xmlfile_for_ber)
    xmlobject.keep_only_chips_by_modulename(module, [chip])
    # xmlobject.keep_only_chips_by_moduleid(1, [chip])
    ...
    for tap0, tap1, tap2 in tap_settings:
        ...
        command = 'CMSITminiDAQ -f %s -c %s %i 2>&1 | tee %s' % (xmlfile_for_ber, tuningstepname, value, os.path.join(logfolder, 'ber_%i_%i_%i.log' % (tap0, tap1, tap2)))
