...
class Parser():
    ...
    def parse(self, data):
        for getter in self._getters: 
            self.info[getter.name] = getter.read_data(data) 
...
class LogFileParser(Parser):
    ...
    def parse(self, logfile_name):
        ...
        self.info[getter.name] = getter.read_data(None)
...
class LogFileGetter(InfoGetter):
    """InfoGetter for getting information out of LogFiles. Initializes to a default value, which can be updated in the read_line method."""
...
class ThresholdGetter(InfoGetter):
    ...
    def get(self, data=None):
        lastfilename = utils.get_last_scan_file('ThrAdjustment')
...
class BERGetter(LogFileGetter):
    ...
    def read_line(self, l):
        if 'Final counter:' in line:
            self._value =  = safe_convert( l.split(' ')[-4], int, 'NBER') 
...
class NFramesGetter(LogFileGetter):
    ...
    def read_line(self, l):
        if 'Final number of PRBS frames sent:' in l:
           self._value = safe_convert( l.split(' ')[-1], int, 'NFrames')
...
class UplinkSpeedGetter(LogFileGetter):
    ...
    def read_line(self, line):
        if 'Up-link speed:' in l:
            speed = safe_convert( l.split(' ')[-2], float, 'Up-link Speed')
...
class InitMaskedPixGetter(ChipSpecificLogFileGetter):
    ...
    def update_values(self, l):
        if 'Number of masked pixels:' in l: #masked pixels
                ...
                self._value[self.chipId] = safe_convert(l.split()[-1], int, 'maskedPixels')
    ...
    def update_values(self, l):
        if 'TEMPSENS_' in l: 
            ...
            if m is not None:
                ...
                if not self._chipId in self._value:
                    self._value[self._chipId] = {}
                self._value[self.chipId][sens] = temp
            else:
    ...
    def update_values(self, l):
        if 'VOUT_' in l: #shuntLDO values
            ...
            if m is not None:
                ...
                if not self._chipId in self._value:
                    self._value[self._chipId] = {}
                self._value[self.chipId][vsource] = voltage
            else:
